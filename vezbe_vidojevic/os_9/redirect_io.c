...
int main() {
    ...
    if (child) { // parent 
        ...
        osAssert(-1 != (bytesRead = read(pipeFds[PIPE_RD], buf, sizeof buf)), "Reading message from child in parent failed");
		/* primljenu poruku stampamo na stdout */
        osAssert(-1 != write(STDOUT_FILENO, buf, bytesRead), "Writing message to stdout failed [parent]");
         ...
         */
        osAssert(-1 != dup2(pipeFds[PIPE_WR], STDOUT_FILENO), "Redirecting child's stdout failed");
