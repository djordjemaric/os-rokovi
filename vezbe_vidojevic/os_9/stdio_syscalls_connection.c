...
int main(int argc, char **argv) {
    ...
    osAssert(NULL != fileDem, "Opening file failed");
	/* iz postojeceg streama dohvatamo fajl deskriptor */
    int fileDemFd = fileno(fileDem);
    osAssert(-1 != fileDemFd, "Getting file descriptor from FILE* failed");
	/* upotrebom fstata nad izvucenim fajl deskriptorom citamo informacije o fajlu */
    struct stat finfo;
    osAssert(-1 != fstat(fileDemFd, &finfo), "Getting info through file descriptor failed");
     ...
     */
    int pipeFds[2];
    osAssert(-1 != pipe(pipeFds), "Opening pipe failed");
	/* forkujemo procese */
    pid_t child = fork();
    osAssert(-1 != child, "Forking child failed");
    ...
    if (child) { // Parent
        ...
        osAssert(NULL != pipeFileStream, "Creating C stream from file descriptor failed (parent)");
        ...
        osAssert(NULL != pipeFileStream, "Creating C stream from file descriptor failed (child)");
		/* citamo poruku od parenta pomocu standardne biblioteke */
        osAssert(NULL != fgets(buf, MAX_MSG_LEN, pipeFileStream), "Getting message failed");
    ...
    osAssert(-1 != wait(NULL), "Waiting for child failed");
