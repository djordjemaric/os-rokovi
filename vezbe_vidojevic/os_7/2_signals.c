 ...
 *
 */
#define _DEFAULT_SOURCE /* WARNING: Without this signal handler is reset to default after one shot. */
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <unistd.h>
...
void osHandleSIGINT(int);

void osHandleSIGTERM(int);

void osHandleSIG(int);
...
int main() {
     ...
     * */
    osAssert(SIG_ERR != signal(SIGINT, osHandleSIGINT), "Setting SIGINT handler failed");
    osAssert(SIG_ERR != signal(SIGTERM, osHandleSIGTERM), "Setting SIGTERM handler failed");
     ...
     *   osAssert(SIG_ERR != signal(SIGINT, osHandleSIG), "Setting SIGINT handler failed");
     *   osAssert(SIG_ERR != signal(SIGTERM, osHandleSIG), "Setting SIGTERM handler failed");
    */
