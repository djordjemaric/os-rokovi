...
prosledjeni fifo fajl ima otvoren odgovarajuci kraj.
*/
...
int main(int argc, char **argv) {
    ...
    if ('r' == argv[2][0]) {
        ...
        osAssert(0, "Wrong checking mode (see usage)");
    }
...
bool osCheckFIFOHasWriteEnd(const char *fifoPath)
{
    ...
    osAssert(-1 != fifoFd, "Opening FIFO for reading failed");
    ...
    if (0 == bytesRead) {
      ...
      if (EAGAIN == errno) {
		 ...
		 */
        osAssert(false, "Read failed when checking if FIFO has writter");
      }
    } else {
...
{	
    ...
    if (-1 == fifoFd) {
		 ...
		 * otvaranja fajla
		 */
        osAssert(0, "Failed to open FIFO for writing");
    }
